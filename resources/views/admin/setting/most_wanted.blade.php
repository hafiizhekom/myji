...
@section('pagetitle', 'Setting Most Wanted Product')
@section('content')
    ...
    data-toolbar="#toolbar">
        ...
        <tbody>
            @foreach ($data['setting_most_wanted'] as $key=>$value)
                <tr>
                    <td>{{$value->id}}</td>
                    <td>{{$value->product->product_name}} {{$value->product->color->color_name}} {{$value->product->category->category_name}}</td>
                    <td>
                        {{$value->order}}
...
@section('modals')
    ...
    <div class="modal fade" id="add" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            <form class="form" action="{{route('setting.most_wanted.add')}}" method="post">
                @csrf
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label>Product</label>
                        <select class="form-control product-detail-add-selectpicker" name="product_id" placeholder="Product" required>
                            @foreach($data['product'] as $key=>$value)
                                <option value="{{$value->id}}">{{$value->product_name}} {{$value->color->color_name}} {{$value->category->category_name}}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <button class="btn btn-primary btn-block" type="submit">Add New Most Wanted Item</button>
                </div>
            </form>
        </div>
        </div>
    ...
    <div class="modal fade" id="edit-{{$value->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form class="form" action="{{route('setting.most_wanted.edit', $value->id)}}" method="post">
                @csrf
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label>Product</label>
                        <select class="form-control product-detail-edit-selectpicker" name="product_id" placeholder="Product" data-id="{{$value->id}}" required>
                            @foreach($data['product'] as $key=>$product)
                                @if($product->id == $value->product->id)
                                    <option value="{{$product->id}}" selected>{{$product->product_name}} {{$product->color->color_name}} {{$product->category->category_name}}</option>
                                @else
                                    <option value="{{$product->id}}">{{$product->product_name}} {{$product->color->color_name}} {{$product->category->category_name}}</option>
                                @endif
                            @endforeach
                        </select>
                    </div>
    ...
    <div class="modal fade" id="increasing-{{$value->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form" action="{{route('setting.most_wanted.increasing.edit', $value->id)}}" method="post">
                    @csrf
    ...
    <div class="modal fade" id="decreasing-{{$value->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form" action="{{route('setting.most_wanted.decreasing.edit', $value->id)}}" method="post">
                    @csrf
                    <div class="btn-group" style="width: 100%;">
    ...
    <div class="modal fade" id="delete-{{$value->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form" action="{{route('setting.most_wanted.delete', $value->id)}}" method="post">
                    @csrf
                    {{ method_field ('DELETE') }}
                    <div class="btn-group" style="width: 100%;">
